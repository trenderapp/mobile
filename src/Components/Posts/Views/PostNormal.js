...
import useVideoProxyCache from "react-native-video-cache"
                ...
                !info?.type || info.type === 0 ?
                    null : info.type === 1 ? 
                        <Carroussel pictures={info.attachments} />
                            : info.type === 2 ?
                                <VideoPlayer thumbnail={info.attachments[0]?.thumbnail ? client.post.file(info.from.user_id, info.post_id, info.attachments[0]?.thumbnail): undefined} uri={`${useVideoProxyCache(client.post.file(info.from.user_id, info.post_id, info.attachments[0]?.name))}`} /> : null
            }
