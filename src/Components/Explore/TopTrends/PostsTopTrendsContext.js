...
export const PostsTopTrendsContext = React.createContext({
    posts: [],
    dispatch: () => {}
});

export const PostsTopTrendsContextProvider = ({ children }) => {
    const [posts, dispatch] = useReducer(reducer, initialState)
    
    return (
        <PostsTopTrendsContext.Provider value={{posts, dispatch}}>
            { children }
        </PostsTopTrendsContext.Provider>
    )
}