        ...
        <>
            <Text style={{ fontSize: 15, }} onPress={undefined} selectable={true} numberOfLines={maxLine ? maxLine + 1 : undefined}>
                <Suspense fallback={content}>
                    <Renderer noBr={noBr ? false : true} maxLine={maxLine} content={maxLine ? `${content.slice(0, 180)}` : content} />
                </Suspense>
                {
                    maxLine ?
                        content.split("\n").length > 5
                            ? <Text style={{ color: "#00B0F4" }}>{t("commons.see_more")}</Text>
                            : content.length > 180 ? <Text style={{ color: "#00B0F4" }}>{t("commons.see_more")}</Text>
                                : null : null
                }
            </Text>
            {
