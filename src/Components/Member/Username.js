        ...
        <View>
            <View>
                ...
                <View style={styles.row}>
                    <Text style={[styles.text_muted, { color: colors.text_muted, marginRight: 5 }]}>@{user?.nickname}</Text>
                    { user?.is_private && <SvgElement margin={5} size={18} name="lock" color={colors.text_normal} /> }
                    { flags.has(userFlags.VERIFIED_USER) && <SvgElement margin={5} name="verified" size={18} /> }
                </View>
            </View>
            <View style={{
