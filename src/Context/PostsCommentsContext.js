...
export const PostsCommentsContextListContext = React.createContext({
    posts: [],
    dispatch: () => {}
});

export const PostsCommentsContextListContextProvider = ({ children }) => {
    const [posts, dispatch] = useReducer(reducer, initialState)
    
    return (
        <PostsCommentsContextListContext.Provider value={{posts, dispatch}}>
            { children }
        </PostsCommentsContextListContext.Provider>
    )
}